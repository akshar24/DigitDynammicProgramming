 ...
 * @param high
 */
const countNumbersWithSuffixInRange__optimal = (suffix, low, high) => {
    const left = __optimal_algorithm__helper(suffix, low - 1);
    const right = __optimal_algorithm__helper(suffix, high);
    return right - left;
};
const __optimal_algorithm__helper = (suffix_as_string, high_as_number) => {
    const suffix_as_number = +suffix_as_string;
...
const countNumbersWithSuffixInRange__bruteForce = (suffix, low, high) => {
    let count = 0;

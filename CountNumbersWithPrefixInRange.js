 ...
 * @param high
 */
const countNumbersWithPrefixInRange__optimal = (prefix, low, high) => {
    const left = __optimal_algorithm_helper(prefix, low - 1);
    const right = __optimal_algorithm_helper(prefix, high);
    return right - left;
};
const __optimal_algorithm_helper = (prefix_as_string, high_as_number) => {
    const prefix_as_number = +prefix_as_string;
    const high_as_string = high_as_number.toString();
 ...
 * @param high
 */
const countNumbersWithPrefixInRange__bruteForce = (prefix, low = 1, high) => {
    let count = 0;
